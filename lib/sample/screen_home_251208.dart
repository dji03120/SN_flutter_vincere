...
class _MyHomePageState extends State<MyHomePage> with SingleTickerProviderStateMixin {
  ...
  bool _isFoodSectionOpen = false;
  bool _isExerciseSectionOpen = false;
  ...
  Future<void> insertRiceIntake(String mealType, double amount) async {
    try {
      ...
      if (result != null) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('섭취량 입력에 실패했습니다.'), backgroundColor: Colors.red),
        );
      }
    } catch (e) {
      print('Error inserting rice intake: $e');
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('오류가 발생했습니다. 다시 시도해주세요.'), backgroundColor: Colors.red),
      );
    }
  }
          ...
          ? SingleChildScrollView(
              child: Column(
                children: [
                  ...
                  TextButton(
                    onPressed: () {
                      setState(() => _isFoodSectionOpen = !_isFoodSectionOpen);
                    },
                    child: Text(
                      _isFoodSectionOpen ? "식단 닫기 ▲" : "식단 열기 ▼",
                                              ...
                                              children: [
                                                ...
                                                Flexible(
                                                  child: Column(
                                                    crossAxisAlignment: CrossAxisAlignment.start,
                                                    children: [
                                                      Text('오늘하루', style: TextStyle(fontWeight: FontWeight.w700, fontSize: constraints.maxWidth > 300 ? 22 : 16, color: Color(0xFFFFFFFF), letterSpacing: -0.02)),
                                                      Text('쌀 섭취량 입력', style: TextStyle(fontWeight: FontWeight.w700, fontSize: constraints.maxWidth > 300 ? 22 : 16, color: Color(0xFFFFFFFF))),
                                                      SizedBox(height: 20),
                                                    ],
                                                  ),
                                                ),
                                          ...
                                          HomeScreenButton(
                                            ...
                                            onPressed: () {
                                              showDialog(
                                                context: context,
                                                builder: (BuildContext context) {
                                                  return MetricChartDialog(title: '쌀 섭취량(g)', code: 'riceIntake', userId: userId);
                                                },
                                              );
                                            },
                                          ),
                  ...
                  TextButton(
                    onPressed: () {
                      setState(() => _isExerciseSectionOpen = !_isExerciseSectionOpen);
                    },
                    child: Text(
                      _isExerciseSectionOpen ? "운동 닫기 ▲" : "운동 열기 ▼",
                      style: const TextStyle(
                        fontWeight: FontWeight.w700,
                        fontSize: 22,
                      ),
