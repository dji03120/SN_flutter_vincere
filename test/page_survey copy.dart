...
class _QuestionDisplayWidgetState extends State<QuestionDisplayWidget> {
  ...
  bool _shouldShowSubQuestion(
    ...
    final String? answerValue =
        questionId == 'self' ? selectedValue : answers[questionId]?.toString();

    if (answerValue == null || conditionValue == null) return false;

    // value: "주 1일|주 2~3일"
    final Set<String> conditionValues = conditionValue
        .toString()
        .split('|')
        .map((e) => e.trim())
  ...
  Widget buildMainInput(Question q) {
          ...
          return Column(
            ...
            children: [
              ...
              if (isSelected)
                ...q.subQuestions
                    .where((sub) =>
                        _shouldShowSubQuestion(q, sub, selected, answers))
                    .expand((sub) {
                  return [
                    Padding(
                      padding: const EdgeInsets.only(
    ...
    if (qType == 'checkbox') {
      final List<String> selectedList = (selected ?? '').toString().isEmpty
          ? []
          : (selected ?? '').toString().split(',').where((s) => s.isNotEmpty).toList();
              ...
              CheckboxListTile(
                ...
                onChanged: (v) {
                  setState(() {
                    ...
                    // 구분자 통일 (데이터 모델에 따라 , 또는 | 선택)
                    answers[qId] = selectedList.join(',');
                  });
                },
              ),

              // ✅ [수정] 다중 선택 시에도 각각의 체크된 항목에 대해 서브 질문을 검사합니다.
              if (checked) 
                ...q.subQuestions
                    .where((sub) {
                      // showIf 조건이 'self'인 경우, 현재 옵션(opt)이 조건값과 일치하는지만 확인합니다.
                      final dynamic conditionValue = sub.showIf['value'];
                      if (conditionValue == null) return false;
                      
                      // 조건값이 여러 개(A|B)일 경우를 대비해 split 처리
                      final Set<String> conditionValues = conditionValue
                          .toString()
                          .split('|')
                          .map((e) => e.trim())
                      ...
                      return conditionValues.contains(opt);
                    })
                    .expand((sub) {
                  return [
                    Padding(
                      ...
                      child: Container(
                        ...
                        padding: const EdgeInsets.all(12.0),
                        decoration: BoxDecoration(
                          color: Colors.blue.shade50, // 가독성을 위해 체크박스 하위는 연한 파란색 적용
                          borderRadius: BorderRadius.circular(8.0),
    ...
    if (qType == 'text_with_input') {
      ...
      String lastUnit = hasClosingParen
          ? lastRaw.substring(0, lastRaw.length - 1)
          : lastRaw;
  ...
  Widget buildDetailSubsIfAny({
    ...
    return Padding(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: detailSubQuestions
          .where((DetailSubQuestion dsq) {
                    ...
                    Builder(builder: (context) {
                      ...
                      return buildDetailSelectableWrap(
                        ...
                        children: items.map((item) {
                          ...
                          final isChecked =
                              (detailAnswers[mainId]![checkedKey] ?? '') == 'Y';
                          ...
                          return Row(
                            ...
                            children: [
                              Checkbox(
                                value: isChecked,
                                onChanged: (v) {
                                  detailAnswers[mainId]![checkedKey] =
                                      v == true ? 'Y' : '';
                                  refresh();
                                },
                              ),
  ...
  Widget buildDetailSelectableWrap({
    ...
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        final itemWidth =
            (availableWidth - spacing * (perRow - 1)) / perRow;
              ...
              child: IconButton(
                ...
                onPressed: () async {
                  ...
                  if (dateType == 'YYYY') {
                    ...
                    if (pickedDate != null) {
                      String formattedDate = dateType == 'YYYY-MM'
        ...
        LayoutBuilder(builder: (context, constraints) {
          ...
          return Wrap(
            ...
            children: subItems.map<Widget>((SubItem item) {
              ...
              detailAnswers[mainId] ??= {};
              final selectedSet = detailAnswers[mainId]![subId] != null
                  ? detailAnswers[mainId]![subId]!.split(',').where((s) => s.isNotEmpty).toSet()
                  : <String>{};
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    buildDetailSubsIfAny(
                      ...
                      visible: checked,
                      detailSubQuestions:
                              (detailSubQuestions ?? []).cast<DetailSubQuestion>(),
                      detailAnswers: detailAnswers,
                      refresh: () => setState(() {}),
      ...
      return [
        ...
        LayoutBuilder(builder: (context, constraints) {
          ...
          if (subMinWidth <= 0) {
            ...
            int calculatedPerRow =
                (availableWidth + spacing) ~/ (subMinWidth + spacing);
            if (calculatedPerRow < 1) calculatedPerRow = 1;
            perRow = calculatedPerRow;
          }
          ...
          return Wrap(
            ...
            children: subItems.map<Widget>((item) {
              ...
              final textController = TextEditingController(
                  text: detailAnswers[mainId]!['${keyValue}_text'] ?? '');
                      ...
                      onCheckedChanged: (v) {
                        setState(() {
                          if (v == true) {
                            ...
                            detailAnswers[mainId]!.removeWhere(
                              (k, _) =>
                                  k.startsWith('$subId|') || k.contains('_dsq_'),
                            );
